...
def detective(history, memory, delay):
    ...
    if num_rounds < len(testing_schedule):  # intitial testing phase
        ...
        if history[1,-1] == 1 and history[1,-2] == 1 and history[1,-3] == 1 and history[1,-4] == 1 and history[1,-5] ==1:  # they never defected, take advantage of them
            choice = 0
            memory = "grudge"
...
def strategy(history, memory):
    ...
    if round == 0:
            return "cooperate", [True, True, False, False, 0, 'null', 200]
    
    new_memory = memory
    
    if history [1, -1] == 0 and new_memory[DELAY] >= round:
        new_memory[DELAY] = round
    
    if new_memory[DELAY] <= (round - 10):
        (choice, new_memory[DETECTIVE])=detective(history, new_memory[DETECTIVE], new_memory[DELAY] + 10)
    
        return choice, new_memory
    
    if new_memory[GRUDGED]:
        return "defect", new_memory
    
    if new_memory[ABSOLUTING] and new_memory[COOLDOWN] >0:
        new_memory[COOLDOWN]-=1
    
        return "cooperate", new_memory
    
    if new_memory[ABSOLUTING] and new_memory[COOLDOWN] ==0:
        new_memory[ABSOLUTING]= False
        sin = 0
        ...
        for i in range (1, 6):
            ...
            if sin < 5:
                new_memory[ABSOLOTION] = True
    
                return "cooperate", new_memory
            else:
                new_memory[GRUDGED] = True
    ...
    if round == 4:
        ...
        for i in range (1, 5):
            ...
            if sin == 4:
                new_memory[ABSOLOTION] = False
    
    if round > 4 and new_memory[COOLDOWN] ==0:
        sin = 0
        ...
        for i in range (1, 5):
             ...
             if sin == 4:
                 if new_memory[ABSOLOTION]:
                     new_memory[COOLDOWN] = 3
                     new_memory[ABSOLOTION] = False
                     new_memory[ABSOLUTING] = True
    
                     return "cooperate", new_memory
                 else:
                     new_memory[COOLDOWN] =- 1
    
    return forgivingCopycat(history), new_memory
